...
func Error(message string) Result {
	return map[string]interface{}{"error": message, "ok": false, "value": ""}
}

func main() {
	var success_result = Ok("Success!")
	var error_result = Error("Something went wrong")
	fmt.Println("Success:", success_result)
	fmt.Println("Error:", error_result)
}
